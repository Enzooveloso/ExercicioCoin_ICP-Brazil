...
function TransferForm() {
  ...
  async function getCanisterPrincipal(){
    const result = await ExercicioCoin_backend.getCanisterPrincipal();
  ...
  */
  async function approveTokens(am) {    
    ...
    try {
      ...
      */
      const result = await ExercicioCoin_icrc1_ledger_canister.icrc2_approve(approveArgs);    
      ...
      await authClient.login({
          ...
          */
          ...
          onSuccess: async () => {  
              ...
              Actor.agentOf(ExercicioCoin_backend).replaceIdentity(
                  authClient.getIdentity()
              );  

              //registra a identidade do usuário no Canister de ledger
              Actor.agentOf(ExercicioCoin_icrc1_ledger_canister).replaceIdentity(
                  authClient.getIdentity()
              );              
  ...
  async function getAccountFromBalance(account) {
    ...
    try {
        if (account != '') {                        
            setFrom(account);
            const result = await ExercicioCoin_backend.getBalance(Principal.fromText(account));
  ...
  async function transferTokens() {        
    ...
    try {
        ...
        /*
        const allowance = await ExercicioCoin_icrc1_ledger_canister.icrc2_allowance({
